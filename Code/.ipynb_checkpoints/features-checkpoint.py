...
def compute_emotions(video_data: pd.DataFrame) -> dict:
    ...
    processed['mean_Disgust'] = ((video_data[' AU09_r'] + video_data[' AU15_r'])/2).mean()
...
def compute_blink_rate(video_data: pd.DataFrame) -> float:
    ...
    seconds = video_data['timestamp2'].values[-1] - video_data['timestamp2'].values[0] # Get the amount of seconds in the given video signal
